...
#include "Histo/HistoAnalyzer/interface/jetValidationWithPf.h"
...
void
jetValidationWithPf::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  const std::vector<float> & myweight=*weight;
   ...
   {   
      ...
      reco::PFCandidateCollection::const_iterator itPf=goodEPair->begin();
      TLorentzVector gsfe1, gsfe2, gsfe_pair;
      ...
      if (doEfficiencies){
	 h_totalPfElectronsVsEn->Fill(itPf->energy(),myweight[0]);
	 h_totalPfElectronsVsEta->Fill(itPf->eta(),myweight[0]);
      }

      for(reco::GsfElectronCollection::const_iterator it=gsfElec->begin();it!=gsfElec->end();it++){
	 if (itPf->gsfTrackRef()==it->gsfTrack()) {

// =================== S T A R T    --  Super Clusters ===============================
	    ...
	    if (it->superCluster().isNonnull() && it->pflowSuperCluster().isNonnull()){
	       ...
	       h_gsfPfSCEtaVsEta->Fill(pfScEta,gsfScEta-pfScEta,myweight[0]);
	       for (int j=0; j< 100; j++){
		  if ( pfScEta> j*deltaEta_ && pfScEta<(j+1)*deltaEta_){
		     for (int k=0; k<100; k++){
			if ( pfScPhi> k*deltaPhi_ && pfScPhi<(k+1)*deltaPhi_){
			   h_gsfPfSCEnVsEtaPhi->Fill((j+0.5)*deltaEta_,(k+0.5)*deltaPhi_,gsfScEn/pfScEn,myweight[0]);
			}
		     }
		  }
	       }
	       if (fabs(pfScEta)<edgeEB){
		  h_gsfPfSCEta_EB->Fill(gsfScEta-pfScEta,myweight[0]);
		  h_gsfPfSCEn_EB->Fill(gsfScEn/pfScEn,myweight[0]);
		  h_gsfPfSCEnVsEn_EB->Fill(pfScEn,gsfScEn/pfScEn,myweight[0]);
		  h_gsfPfSCEtaVsEn_EB->Fill(pfScEn,gsfScEta-pfScEta,myweight[0]);
	       } else if (fabs(pfScEta)<edgeEE){
		  h_gsfPfSCEta_EE->Fill(gsfScEta-pfScEta,myweight[0]);
		  h_gsfPfSCEn_EE->Fill(gsfScEn/pfScEn,myweight[0]);
		  h_gsfPfSCEnVsEn_EE->Fill(pfScEn,gsfScEn/pfScEn,myweight[0]);
		  h_gsfPfSCEtaVsEn_EE->Fill(pfScEn,gsfScEta-pfScEta,myweight[0]);
...
// =================== E N D   --  Super Clusters ===============================

	    gsfSize++;

	    if (doEfficiencies){
	       h_PairedPfElectronsVsEn->Fill(itPf->energy(),myweight[0]);
	       h_PairedPfElectronsVsEta->Fill(itPf->eta(),myweight[0]);
	    }

	    h_ptPFptVsEta->Fill(itPf->eta(),it->energy()/itPf->energy(),myweight[0]);
	    h_ptPFptVsEn->Fill(itPf->energy(),it->energy()/itPf->energy(),myweight[0]);
	    double ratioPt =  (it->pt()-itPf->pt())/it->pt();
	    h_ptGsfPfGsfVspt->Fill(itPf->pt(),ratioPt,myweight[0]);
	    gsfe1.SetPtEtaPhiM(it->pt(),it->eta(),it->phi(),it->mass());
	    checkPf1=true;
	    ...
	    if (usingMC){
		 ...
		 dist= sqrt( pow(itgen->eta()-itPf->eta(),2)+pow(itgen->phi()-itPf->phi(),2) );
		 distEta = fabs(itgen->eta() - itPf->eta());
...
//========================= E N D  --  SUPER CLUSTERS ================================
	    ...
	    double ratioPt =  (it->pt()-itPf->pt())/it->pt();
	    h_ptGsfPfGsfVspt->Fill(itPf->pt(),ratioPt,myweight[0]);
	    gsfe2.SetPtEtaPhiM(it->pt(),it->eta(),it->phi(),it->mass());
	    checkPf2=true;
      ...
      h_sizeGsf->Fill(gsfSize,myweight[0]);
      gsfe_pair=gsfe1 + gsfe2;
      // ================================
    
      ///////////////////
      /// Jet ValidationWithPf
      ///////////////////
	 ...
	 //
	    ...
	    double deltaR1= sqrt( pow(jet->eta()-pfe1.Eta(),2)+pow(jet->phi()-pfe1.Phi(),2) );
	    double deltaR2= sqrt( pow(jet->eta()-pfe2.Eta(),2)+pow(jet->phi()-pfe2.Phi(),2) );
...
//------------------------------------------------------------------------------------------------- 
      ...
      {
	 double deltaR1= sqrt( pow(pfCand->eta()-pfe1.Eta(),2)+pow(pfCand->phi()-pfe1.Phi(),2) );
	 double deltaR2= sqrt( pow(pfCand->eta()-pfe2.Eta(),2)+pow(pfCand->phi()-pfe2.Phi(),2) );	   
	 ...
	 if (doElectronsComparisonMC){
	    ...
	    {
	       double deltaR1= sqrt( pow(itgen->eta()-pfe1.Eta(),2)+pow(itgen->phi()-pfe1.Phi(),2) );
	       double deltaR2= sqrt( pow(itgen->eta()-pfe2.Eta(),2)+pow(itgen->phi()-pfe2.Phi(),2) );	   
...
//-------------------------------------------------------------------------------------------------
	 ...
	 if (numberOfVertices==(v+1)) h_nJetVtx_EE[v]->Fill(nJetsEE,myweight[0]);
      ...
      double zPt = pfe_pair.Pt();
      double zInvMassPf = pfe_pair.M();
      double zInvMassGsf = gsfe_pair.M();

      //////////////////
      //Filling the Unfolding rootuple!
      //////////////////
      Jet_multiplicity=totJets;
      Z_y=pfe_pair.Rapidity();
      Z_pt=zPt;
	...
	if (numbOfJets<100) Jet_multiplicity_gen=numbOfJets;
      ...
      h_invMassPF->Fill(zInvMassPf,myweight[0]);
      h_massMinusPdgPf->Fill(zInvMassPf-zMassPdg,myweight[0]);
      if (fabs(pfe1.Eta())<=edgeEB && fabs(pfe2.Eta())<=edgeEB) {h_massMinusPdgPf_EB->Fill(zInvMassPf-zMassPdg,myweight[0]);}
      else if (fabs(pfe1.Eta())<=edgeEE && fabs(pfe2.Eta())<=edgeEE &&
	       fabs(pfe1.Eta())>edgeEB && fabs(pfe2.Eta())>edgeEB) { h_massMinusPdgPf_EE->Fill(zInvMassPf-zMassPdg,myweight[0]);}
      else if (fabs(pfe1.Eta())<edgeEE && fabs(pfe2.Eta())<edgeEE) {h_massMinusPdgPf_EBEE->Fill(zInvMassPf-zMassPdg,myweight[0]);}

      if (checkPf1 && checkPf2){
	 h_invMassGSF->Fill(gsfe_pair.M(),myweight[0]);
	 h_massMinusPdgGsf->Fill(gsfe_pair.M()-zMassPdg,myweight[0]);
	 if (fabs(gsfe1.Eta())<=edgeEB && fabs(gsfe2.Eta())<=edgeEB) {h_massMinusPdgGsf_EB->Fill(zInvMassGsf-zMassPdg,myweight[0]);}
	 else if (fabs(gsfe1.Eta())<=edgeEE && fabs(gsfe2.Eta())<=edgeEE &&
		  fabs(gsfe1.Eta())>edgeEB && fabs(gsfe2.Eta())>edgeEB) {h_massMinusPdgGsf_EE->Fill(zInvMassGsf-zMassPdg,myweight[0]);}
	 else if (fabs(gsfe1.Eta())<edgeEE && fabs(gsfe2.Eta())<edgeEE) {h_massMinusPdgGsf_EBEE->Fill(zInvMassGsf-zMassPdg,myweight[0]);}
      }

      h_zEta->Fill(pfe_pair.Eta(),myweight[0]);
      h_zRapidity->Fill(pfe_pair.Rapidity(),myweight[0]);
      ...
      if (totJets == 0){ 
	h_zEtaNjet0->Fill(pfe_pair.Eta(),myweight[0]);
	h_zMassNjet0->Fill(zInvMassPf,myweight[0]);
	if (checkPf1 && checkPf2) h_zMassMinusPdgGsfNjet0->Fill(zInvMassGsf-zMassPdg,myweight[0]);
	h_zMassMinusPdgPfNjet0->Fill(zInvMassPf-zMassPdg,myweight[0]);
      }
      if (totJets == 1){ 
	h_zEtaNjet1->Fill(pfe_pair.Eta(),myweight[0]);
	h_zMassNjet1->Fill(zInvMassPf,myweight[0]);
	if (checkPf1 && checkPf2) h_zMassMinusPdgGsfNjet1->Fill(zInvMassGsf-zMassPdg,myweight[0]);
	h_zMassMinusPdgPfNjet1->Fill(zInvMassPf-zMassPdg,myweight[0]);
      } else if (totJets == 2){ 
	 h_zEtaNjet2->Fill(pfe_pair.Eta(),myweight[0]);
	 h_zMassNjet2->Fill(zInvMassPf,myweight[0]);
	 if (checkPf1 && checkPf2) h_zMassMinusPdgGsfNjet2->Fill(zInvMassGsf-zMassPdg,myweight[0]);
	 h_zMassMinusPdgPfNjet2->Fill(zInvMassPf-zMassPdg,myweight[0]);
      } else if (totJets == 3){ 
	 h_zEtaNjet3->Fill(pfe_pair.Eta(),myweight[0]);
	 h_zMassNjet3->Fill(zInvMassPf,myweight[0]);
	 if (checkPf1 && checkPf2) h_zMassMinusPdgGsfNjet3->Fill(zInvMassGsf-zMassPdg,myweight[0]);
	 h_zMassMinusPdgPfNjet3->Fill(zInvMassPf-zMassPdg,myweight[0]);
      } else if (totJets == 4){ 
	 h_zEtaNjet4->Fill(pfe_pair.Eta(),myweight[0]);
	 h_zMassNjet4->Fill(zInvMassPf,myweight[0]);
	 if (checkPf1 && checkPf2) h_zMassMinusPdgGsfNjet4->Fill(zInvMassGsf-zMassPdg,myweight[0]);
	 h_zMassMinusPdgPfNjet4->Fill(zInvMassPf-zMassPdg,myweight[0]);
      } 

      if (totJets >= 1) h_zEtaNjet1Incl->Fill(pfe_pair.Eta(),myweight[0]);
      if (totJets >= 2) h_zEtaNjet2Incl->Fill(pfe_pair.Eta(),myweight[0]);
      if (totJets >= 3) h_zEtaNjet3Incl->Fill(pfe_pair.Eta(),myweight[0]);
      if (totJets >= 4) h_zEtaNjet4Incl->Fill(pfe_pair.Eta(),myweight[0]);
...
jetValidationWithPf::endJob() 
{
 ...
 h_ptGsfPfGsfVspt->GetYaxis()->SetTitle("(p_{T}^{GSF} - p_{T}^{PF}) / p_{T}^{GSF}");
 h_ptGsfPfGsfVspt->GetXaxis()->SetTitle("p_{T}^{PF}");
 h_totalPfElectronsVsEn->GetXaxis()->SetTitle("PFelectron energy");
 h_totalPfElectronsVsEn->GetYaxis()->SetTitle("N_{electrons}");
 h_PairedPfElectronsVsEn->GetXaxis()->SetTitle("PFelectron energy");
 h_PairedPfElectronsVsEn->GetYaxis()->SetTitle("N_{electrons}");
 h_totalPfElectronsVsEta->GetXaxis()->SetTitle("PFelectron eta");
 h_totalPfElectronsVsEta->GetYaxis()->SetTitle("N_{electrons}");
 ...
 h_invMassGSF->GetXaxis()->SetTitle("mass_{Z}");
 h_invMassGSF->GetYaxis()->SetTitle("N_{Z}");
...
void 
jetValidationWithPf::beginRun(edm::Run const & iRun, edm::EventSetup const& iSetup)
{
}

// ------------ method called when ending the processing of a run  ------------
void 
jetValidationWithPf::endRun(edm::Run const&, edm::EventSetup const&)
{
}

// ------------ method called when starting to processes a luminosity block  ------------
void 
jetValidationWithPf::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void 
jetValidationWithPf::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

//define this as a plug-in
DEFINE_FWK_MODULE(jetValidationWithPf);
