...
string plotpath;
string plotpathEle	="plotEleXSecUP"+versionPath+"/"; //put here the path where you want the plots
string plotpathMu    	="plotMuXSecUP"+versionPath+"/"; //put here the path where you want the plots
//string plotpathEle	="plotEleXSecUP50_v2_33/"; //put here the path where you want the plots
//string plotpathMu    	="plotMuXSecUP50_v2_33/"; //put here the path where you want the plots
...
void DrawComparisonJetMCDataScaleUp(void){
  struct STestFunctor {
  ...
  outfilename=plotpath+"ratioPlotBgScaleUp"+version;
...
//   TFile *qcd817bcf = TFile::Open(qcd817bc.c_str());
  ...
  if (flag==1){
    ...
    if(str.Contains("invMass")) data->GetXaxis()->SetRangeUser(71,111);	
    data->SetMinimum(1.);
    data->Sumw2();
    data->Draw("E1");
...
//     }
    ...
    //hs->Draw("nostack");
    //hs->Draw();
    //TH1D * hsProfile = (TH1D*)hs->GetHistogram();
