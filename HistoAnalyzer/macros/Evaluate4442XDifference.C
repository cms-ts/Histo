...
TFile *CMSSW44 = TFile::Open("rfio:/gpfs/cms/data/2011/jet/jetValidation_DATA_2011A_v2_11.root"); 
TFile *CMSSW42 = TFile::Open("rfio:/gpfs/cms/data/2011/jet/jetValidation_DATA_2011A_v1_11.root"); 
std::string plotpath ="/afs/infn.it/ts/user/marone/html/ZJets/RunA2011_44Vs42/"; //put here the path where you want the plots
std::string whichdirectory="validationJEC";

using namespace std;

void Evaluate4442XDifference(string plot, string directory);
...
void DrawDifferences(void){
  ...
  for (unsigned int j=0; j<3; j++){
    ...
    while ( tobj = iter.Next() ) {
      ...
      Evaluate4442XDifference(name,whichdirectory.c_str());
      //}
      i++;
      //if(i==1)break;
    }
