...
TLatex* CMSPrel(Float_t Lumi,TString _decaychannel,double x, double y){
  ...
  latexLabel->DrawLatex(x,y,"CMS SMP-VJ Preliminary");
  latexLabel->DrawLatex(x,y-0.04,"#sqrt{s} = 7 TeV");
  latexLabel->DrawLatex(x,y-0.08,"anti-k_{T} (R = 0.5) PF Jets ");
  //latexLabel->DrawLatex(x,0.18,(TString)Form("#int Ldt = %.3f fb^{-1}",Lumi));
  latexLabel->DrawLatex(x,y-0.13,"#int Ldt = 4.890 fb^{-1}");
  latexLabel->DrawLatex(x,y-0.18,"Z#rightarrow ee channel");
  latexLabel->DrawLatex(x,y-0.22,_decaychannel);
