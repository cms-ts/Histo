...
string filename=direct+"UnfoldedVJets2011Distributions_v2_35.root";//+version;
...
UnfoldingVJets2011::Loop()
{
  if (isMu) {
    ...
    filename=direct+"UnfoldedVJets2011DistributionsMu_v2_35.root";//+version;
  }
...
//Open MC and data files to retrieve effciencies
  ...
  LoopVJets(1,whichtype, whichalgo);
      
  for (int j=1; j<=3 ;j++){
    if (j==1) whichtype=="Pt";
    if (j==2) whichtype=="Eta";
    if (j==3) whichtype=="Ht";
    for (int i=1; i<=4; i++){
      int numbOfJetsForLoop=i;
      LoopVJets(numbOfJetsForLoop,whichtype, whichalgo);
    }
  }
}
