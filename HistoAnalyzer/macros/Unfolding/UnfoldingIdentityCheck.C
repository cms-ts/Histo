...
UnfoldingIdentityCheck::Loop ()
{
  ...
  getline(cin, method);
  cout<<endl;
  cout<<"Wonna save png images? [y/N]"<<endl;
  string saveimage="";
  getline(cin, saveimage);

  ////////////
  //// Set Parameters
  ///////////

  cout<<"Insert K_bayesian, K_SVD parameter...(default 3)"<<endl;
  int myNumber=3;
  std::cin >> myNumber;
  stringstream num;
  num<<myNumber;
  string title="Using "+method+" with K="+num.str();
  cout<<title<<endl;
  ...
  if (method=="Svd"){
    RooUnfoldSvd unfold_N (&response_N, NMCreco, 5);	// OR
    RooUnfoldSvd unfold_pT (&response_pT, PMCreco, 10);
    RooUnfoldSvd unfold_y (&response_y, yMCreco, 25);
    NReco = (TH1D *) unfold_N.Hreco ();
    PReco = (TH1D *) unfold_pT.Hreco ();
    yReco = (TH1D *) unfold_y.Hreco ();
  }
  ...
  c->cd ();
  string title2="Number of jet + Z distribution. "+title;
  ...
  c->cd ();
  if (saveimage=="Y" || saveimage=="y") c->Print(title3.c_str());
  ...
  PReco->SetTitle ("");
  max=PReco->GetMaximum();
  min=PReco->GetMinimum();
  ...
  d->cd ();
  title3="/afs/infn.it/ts/user/marone/html/ZJets/Unfolding//IdentityCheck/ZptIdentityCheck_"+method+"_"+num.str()+".png";
  if (saveimage=="Y" || saveimage=="y") d->Print(title3.c_str());
  ...
  yReco->SetTitle ("");
  max=yReco->GetMaximum();
  min=yReco->GetMinimum();
  ...
  e->cd ();
  title3="/afs/infn.it/ts/user/marone/html/ZJets/Unfolding/IdentityCheck/ZyIdentityCheck_"+method+"_"+num.str()+".png";
  if (saveimage=="Y" || saveimage=="y") e->Print(title3.c_str());
}
