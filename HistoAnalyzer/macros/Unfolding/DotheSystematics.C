...
/////////////////////////
 ...
 using	std::cout;
  ...
  //  string pathFile="/gpfs/cms/data/2011/jet/jetValidation_zjets_magd_2011"+version+".root";
  string pathFile="/gpfs/cms/data/2011/jet/jetValidation_zjets_magd_2011Mu_v2_52_PU.root";
  TFile *f = TFile::Open(pathFile.c_str());
...
DotheSystematicsUnfMethodJetPt(whichjet){
  ...
  string namejet="";
   ...
   string pathFile="/gpfs/cms/data/2011/Unfolding/UnfoldedVJets2011DistributionsPreapproval3_v2_35.root"; //SVD
   TFile *f = TFile::Open(pathFile.c_str());

  f->cd(); //wrong one!!! need to have the dir. w/o JEC
  TH1F *MD;
  if (whichjet==1) MD = (TH1F*)gDirectory->Get("HReco_leading");
  if (whichjet==2) MD = (TH1F*)gDirectory->Get("HReco_subleading");
  if (whichjet==3) MD = (TH1F*)gDirectory->Get("HReco_subsubleading");
  if (whichjet==4) MD = (TH1F*)gDirectory->Get("HReco_subsubsubleading");

  string UnfMethod;
  TFile *f2;
  UnfMethod="/gpfs/cms/data/2011/Unfolding/UnfoldedVJets2011DistributionsPreapproval3Bayes_v2_35.root";
  ...
  if (whichjet==1) P = (TH1F*)gDirectory->Get("HReco_leading");  
  if (whichjet==2) P = (TH1F*)gDirectory->Get("HReco_subleading");  
  if (whichjet==3) P = (TH1F*)gDirectory->Get("HReco_subsubleading");  
  if (whichjet==4) P = (TH1F*)gDirectory->Get("HReco_subsubsubleading");  
