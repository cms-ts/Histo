...
makeZjetsPlots (int whichobservable, int whichjet, int whichlepton, bool inclusiveMultiplicity)
{
  ...
  while ((tobj = iter.Next ()))
	...
	TFile *histoRivetPowhegUP = TFile::Open (rivetPathPowhegUP.c_str ());
	histoRivetPowhegUP->cd ("");
	TDirectory *dirRivetPowhegUP = gDirectory;
	TList *mylistRivetPowhegUP = (TList *) dirRivetPowhegUP->GetListOfKeys ();
	TIter iterRivetPowhegUP (mylistRivetPowhegUP);
	TObject *tobjRivetPowhegUP = 0;
	...
	TDirectory *dirRivetPowhegDOWN = gDirectory;
	TList *mylistRivetPowhegDOWN = (TList *) dirRivetPowhegDOWN->GetListOfKeys ();
	TIter iterRivetPowhegDOWN (mylistRivetPowhegDOWN);
	TObject *tobjRivetPowhegDOWN = 0;
...
//	  statTest->SetPoint(ovo,dummyXvar,sqrt(leadingRivetSherpaDOWN->GetErrorYhigh(ovo)*leadingRivetSherpaDOWN->GetErrorYhigh(ovo) + leadingRivetSherpaUP->GetErrorYhigh(ovo)*leadingRivetSherpaUP->GetErrorYhigh(ovo))/fabs(y1temp-y2temp));//stattest
	...
	Double_t dummyNorm = 0.;
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  leadingRivetPowhegUP->GetPoint(ovo,dummyXvar,dummyYvar); 
	  dummyNorm = dummyNorm + dummyYvar;
	}
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  if (absoluteNormalization) {
	  ...
	  leadingRivetPowhegUP->GetPoint(ovo,dummyXvar,dummyYvar);
	  leadingRivetPowhegUP->SetPoint(ovo,dummyXvar,dummyYvar/dummyNorm); 
	  leadingRivetPowhegUP->SetPointEYhigh(ovo,leadingRivetPowhegUP->GetErrorYhigh(ovo)/dummyNorm);
	  leadingRivetPowhegUP->SetPointEYlow(ovo,leadingRivetPowhegUP->GetErrorYlow(ovo)/dummyNorm);
	}

	// Powheg Down
	Double_t dummyNorm = 0.;
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  leadingRivetPowhegDOWN->GetPoint(ovo,dummyXvar,dummyYvar); 
	  dummyNorm = dummyNorm + dummyYvar;
	}
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  if (absoluteNormalization) {
	  ...
	  leadingRivetPowhegDOWN->GetPoint(ovo,dummyXvar,dummyYvar); 
	  leadingRivetPowhegDOWN->SetPoint(ovo,dummyXvar,dummyYvar/dummyNorm); 
	  leadingRivetPowhegDOWN->SetPointEYhigh(ovo,leadingRivetPowhegDOWN->GetErrorYhigh(ovo)/dummyNorm);
	  leadingRivetPowhegDOWN->SetPointEYlow(ovo,leadingRivetPowhegDOWN->GetErrorYlow(ovo)/dummyNorm);
	}
